...
class OutlierHandler:
    def __init__(self, columns_to_check=None, iqr_factor=1.5):
        ...
        if columns_to_check is None:
            self.columns_to_check = [numerical_features]
        else:
    ...
    def transform(self, X):
        ...
        logging.info(f"Handling outliers for columns: {self.columns_to_check}")
...
class DataPreparation:
    ...
    def _detect_outliers(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        # Retrieve numerical features from the configuration
        numerical_features = self.config["columns"]["numerical_features"]
        
        # Check if numerical features are in the DataFrame columns
        if not all(feature in df.columns for feature in numerical_features):
            raise ValueError(f"The following columns are missing: {', '.join(feature for feature in numerical_features if feature not in df.columns)}")
            
        logging.info("Detecting and handling outliers.")        
        # Initialize the OutlierHandler and transform the data    
